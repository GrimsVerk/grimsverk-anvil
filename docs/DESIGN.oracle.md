...
## OD-4 — BL-5 resolved: OD-1's measurement lands in halves, each with the plan that builds what it observes

- **Date:** 2026-08-20
- **Evidence:** BL-5
- **Requirements added:** (none)
- **Requirements superseded:** (none)
- **Vision statement relied on:** the durable-evidence statement (unnumbered, third bullet under *Durable evidence is a requirement, not a nicety*) — "When a decision alters behaviour that no existing check, test, run report or review artifact would notice, adding the thing that notices is part of the decision — not a follow-up, and not optional." The sentence binds a measurement to the change it notices, and that assignment is the whole question BL-5 asks: the unit tests notice the formatting rule, so they land with the plan that builds the rule; the S1 and S2 scripts notice the installed command's exact stdout, so they land with the plans that build that command.
- **Vision statements against:** V6 — "A conversion that is numerically wrong, however plausible it looks, is rejected outright — as is any run that reports success while a criterion's evidence cell is empty or narrated rather than executed." Its second half is the nearest bar to deferring the acceptance scripts, and it is not crossed: nothing here reports success. The S1 and S2 rows in docs/acceptance.md stay pending with empty evidence cells, `.github/scripts/acceptance-criteria.sh` lists every unscripted criterion on every pull request, and no run can report success while those rows are empty — V6 itself forbids it. Deferral moves the executed evidence to the first commit at which execution is possible; landing a script before the command exists would produce not executed evidence but an unconditional failure that blocks the very build that would create the evidence.
- **Alternatives considered:** Landing acceptance/S1.sh and S2.sh from the R1000 steward plan, as OD-1's clause literally reads — impossible without inventing the command they invoke: writing S1.sh now means deciding the exact CLI syntax, which OD-1 explicitly left to a later planner and no logged evidence decides, and a landed script invoking a command nobody built exits non-zero, turning the required acceptance-criteria check red on its own pull request and every one after until the CLI merges; the check's own header says failing the pipeline for unbuilt work "would stop the build that would make it pass". Widening the steward plan to build a minimal `anvil` CLI so the scripts have a subject — that is the `convert` milestone wearing a steward plan's clothes: it moves every slice boundary, forces the same syntax ruling sideways, and AGENTS.md forbids widening a plan to smuggle work past the design chain. Placeholder scripts that exit 0 until the command exists — strictly worse than no script: acceptance-criteria.sh reports a missing script honestly on every pull request, while a green no-op converts that visible gap into a measurement of nothing, which is the narration hole the scripts exist to close. Waiving S1 and S2 until the commands exist — a waiver is for a criterion met by means its script cannot see; these criteria are not met at all, and the check already handles not-yet-built criteria by reporting rather than failing, so a waiver would spend the one field that changes a gate on a problem the gate does not have.
- **Rationale:** BL-5's proposed default is adopted, because it is the durable-evidence statement applied twice: each half of OD-1's measurement lands with the plan that builds the thing that half observes. The R1000 steward plan delivers the formatting rule and its blind unit tests asserting exact printed strings — including the ESC-1 reproduction, formatting the 0.1 km → m artifact value to exactly `100`, red against the artifact and green against the fix — and neither builds a command-line path nor decides the CLI syntax. The acceptance scripts land with the milestone plans that build the command they invoke: S1 with milestone `convert`, S2 with milestone `temperature`, each carrying OD-1's exact-stdout-string constraint verbatim and S1 carrying the 0.1 km → m row in its fixed table; no acceptance script lands before the command it runs exists. Until then the gap stays visible rather than silent, on every pull request, through the check's own "no script yet" report. Confidence is high: every alternative either turns a required check red against unbuilt work or decides an open design question sideways.

**What this supersedes.** One sentence of OD-1's measurement prose — "The plan
that covers R1000 must make the rule observable twice" — is superseded insofar
as it reads as a single plan delivering both halves. Everything else in OD-1
stands unchanged: R1000 itself, the content of both measurements, and the
exact-string-comparison rule. The obligation now distributes: the plan covering
R1000 delivers the unit-test half; the `convert` and `temperature` milestone
plans deliver `acceptance/S1.sh` and `acceptance/S2.sh` respectively, in the
same plan as the command each script invokes.

**Downstream.** Two consequences. First, the ESC-1 correction row waits for the
script: the escape's stub names an acceptance script as its pending check, so
the row is appended by the `convert` implementer once `acceptance/S1.sh` has
landed and run green, citing the demonstrated check per docs/escapes.md's own
rules — not by the R1000 steward plan, whose unit tests are necessary but are
not the check the stub is waiting on. Second, the exact CLI syntax and the
batch line format remain open exactly as OD-1 left them; the `convert` planner
files them as uncertainties when planning reaches them, and nothing in this
decision narrows the answer.
